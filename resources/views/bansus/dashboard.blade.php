    ...
    <div class="w-full px-6 md:px-8 lg:px-12">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-6 mb-10 mt-4">
            <div class="">
                ...
                <div class="mt-3 text-gray-500 text-lg font-normal">Kelola pemesanan laboratorium dengan mudah dan efisien.
                </div>
            </div>
            <div class="flex flex-col md:flex-row gap-3 items-start md:items-center pt-2">
                <a href="{{ route('bansus.bookings.index') }}"
                    class="flex items-center px-6 py-3 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-xl shadow-md transition">
                    <svg class="w-5 h-5 mr-2 opacity-80" fill="none" stroke="currentColor" stroke-width="2"
                        viewBox="0 0 24 24">
                ...
                <a href="{{ route('bansus.users.index') }}"
                    class="flex items-center px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-xl shadow-md transition">
                    <svg class="w-5 h-5 mr-2 opacity-80" fill="none" stroke="currentColor" stroke-width="2"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5V4H2v16h5m10 0v-4a2 2 0 00-2-2H9a2 2 0
                         00-2 2v4h10zM7 10a2 2 0 100-4 2 2 0 000 4z" />
                    </svg>
                    Data User
                </a>
            </div>
                        ...
                        <div class="flex justify-between px-8 py-5 items-start hover:bg-gray-50 transition">
                            <div class="flex-1">
                                <div class="flex gap-2 items-center mb-2 flex-wrap">
                                    <span
                                        class="px-3 py-1.5 rounded-lg text-xs bg-blue-100 text-blue-700 font-bold">{{ $booking->lab->nama ?? 'Lab Dihapus' }}</span>
                                    @if ($booking->status === 'pending')
                                        <span
                                            class="px-3 py-1.5 rounded-full text-xs bg-yellow-100 text-yellow-800 font-semibold">Pending</span>
                                    @elseif($booking->status === 'approved')
                                        <span
                                            class="px-3 py-1.5 rounded-full text-xs bg-green-100 text-green-800 font-semibold">Disetujui</span>
                                    @else
                                        <span
                                            class="px-3 py-1.5 rounded-full text-xs bg-red-100 text-red-800 font-semibold">Ditolak</span>
                                    @endif
                                </div>
                                <div class="font-semibold text-base text-gray-900 mb-1">{{ $booking->user->name }}</div>
                                <div class="text-sm text-gray-500">{{ $booking->tanggal->format('d/m/Y') }} ·
                                    {{ $booking->waktu }}</div>
                            </div>
                            <a href="{{ route('bansus.bookings.show', $booking) }}"
                                class="text-blue-600 font-semibold hover:text-blue-800 text-sm ml-4 whitespace-nowrap">Lihat
                                Detail</a>
                        </div>
                    @empty
                    ...
                    @forelse($todayBookings as $booking)
                        <div class="flex justify-between px-8 py-5 items-start hover:bg-gray-50 transition">
                            <div class="flex-1">
                                ...
                                @if ($booking->status === 'pending')
                                    <span
                                        class="px-3 py-1.5 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800 inline-block">Pending</span>
                                @elseif($booking->status === 'approved')
                                    <span
                                        class="px-3 py-1.5 rounded-full text-xs font-semibold bg-green-100 text-green-800 inline-block">Disetujui</span>
                                @else
                                    <span
                                        class="px-3 py-1.5 rounded-full text-xs font-semibold bg-red-100 text-red-800 inline-block">Ditolak</span>
                                @endif
                            </div>
                        </div>
                    @empty
